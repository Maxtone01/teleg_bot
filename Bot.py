...
from Buttons import testkboard, keyboard1, keyboard2, keyboard3, keyboard4
...
def default_test(message):
    ...
    bot.send_message(message.chat.id, "По нажатию кнопки вы перейдете на сайт с официальной документацией.", reply_markup=keyboard)
...
def start_message(message):
    ...
    '/docs - Предложит перейти вам на сайт с официальной документацией по языку \n /lections - Лекции по пайтону. \n '
    '/test - Пройти тест. \n '
    'Так же чтобы повысить ваши навыки программирования, я предлагаю вам читать исхожные коды других программистов. Чтобы перейти на сайт гитхаба, выберите команду /hub')
...
def start_message(message):
    ...
    kb.add(url_button)
    bot.send_message(message.chat.id, "По нажатию кнопки вы перейдете на сайт. Рекомендую вам зарегестрироваться, если вы этого еще не сделали, чтобы загружать туда свои работы для наполнения вашего будущего резюме",reply_markup=kb)
...
def send_text(message):
    if message.text == 'Привет':
        ...
        bot.send_message(message.chat.id, 'Объектно-ориентированное программирование (ООП) — методология программирования, основанная на представлении программы в виде совокупности объектов, каждый из которых является экземпляром определённого класса, а классы образуют иерархию наследовния')
...
def longname(call):
    ...
    if call.data == "Дальше":
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=keyboard3())
    if call.data == "Следующая":
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=keyboard4())
    if call.data == "В начало":
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=keyboard1())
